...
class Program
{
        ...
        while (continuar == true) //while até escolher 6
        {
            ...
            switch (n) //switch que lava ao procedimento, conforme a opção escolhida
            {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    string filePath = "C://vendas.txt"; //caso ja tenha um arquivo preparado, ele irá apontar e perguntar se quer ou não fazer outro, caso não tenha irá fazer um
    ...
    {
        Console.Clear();
        string filePath = "C://vendas.txt";

        if (File.Exists(filePath))//caso não tenha o arquivo ele irá pedir para fazer a opc 5 primeiro
        {
            StreamReader sr = new StreamReader("C://vendas.txt");//abre arquivo para ler e receber
            ...
            { //Mostra os produtos que tem e o estoque
            ...
            int n = 0;
            for (int i = 0; i < produtos.Length; i++)
            {//escolher qual produto vai mudar o estoque
    ...
    static void registrarVenda() //registra a venda
    {
        ...
        if (File.Exists("C://vendas.txt"))
        { //caso não tenha o arquivo ele irá pedir para fazer a opc 5 primeiro
            StreamReader sr = new StreamReader("C://vendas.txt");
            string produtoLine = sr.ReadLine();
            string estoqueLine = sr.ReadLine();
            string mtzLine = sr.ReadLine();
            ...
            while (n > nn)
            {//while até digitar o numero do produto
                Console.WriteLine("Digite o número referente ao produto vendido.");
                for (int i = 0; i < produtos.Length; i++)
                { //for para escrever todas as informações de produtos e do estoque
                    Console.WriteLine($"{i + 1} - {produtos[i]}.");
                }
            ...
            if (estoque[n] < 1)
            { //ve se não ja está como zero o estoque do produto escolhido
            ...
            else
            {
                ...
                if (nu > estoque[n])
                { //ve se não é maior do que a no estoque
                ...
                { //dimuniu a qtd
                    ...
                    StreamWriter sw = new StreamWriter("C://vendas.txt", false, Encoding.ASCII);//atualiza arquivo
                    sw.WriteLine(string.Join(",", produtos));
                    sw.WriteLine(string.Join(",", estoque));
                    sw.WriteLine(string.Join(",", mtz));
                    sw.Close();
    ...
    {
        Console.Clear();
        string filePath = "C://vendas.txt"; //caso não tenha o arquivo ele irá pedir para fazer a opc 5 primeiro
    ...
    static void relatorioEstoques() //Mostra o estoque atualizado
    {
        ...
        if (File.Exists(filePath)) //caso não tenha o arquivo ele irá pedir para fazer a opc 5 primeiro
        {
            if (File.Exists(filePath))
            {
                ...
                for (int i = 0; i < produtos.Length; i++)
                { //for para mostrar os produtos e seus respectivos estoques
                    Console.WriteLine($"Produto: {produtos[i]}, Estoque: {estoque[i]}.");
                }
                Console.WriteLine("Pressione enter, para continuar.");
                Console.ReadKey();
    ...
    {
        StreamWriter sw = new StreamWriter("C://vendas.txt", false, Encoding.ASCII);
